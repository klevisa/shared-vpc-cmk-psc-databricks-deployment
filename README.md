...
| Step | Prereqs | Teams | Privileges | Output |
|---|---|---|---|---|
| **[2.1 Create service project](workspace-setup/service-project/README.md)** | 1.1 + GCP host project exists | GCP Cloud Foundations | **GCP:** project creator, billing, Shared VPC admin, role admin (org/folder) | Service project created (no default network, break-glass Owner only); APIs enabled; attached to Shared VPC host; service-project number + GCS service agent; **dedicated node SA (no project roles)**; read-only workspace-creator role (service) |
| **[2.2 Create network](workspace-setup/network/README.md)** | service project exists | GCP Network Engineering | **GCP:** network + firewall + DNS + role admin (host project) | VPC + node/PSC subnets, firewall, two PSC endpoints (PENDING), private DNS zone, subnet grants; read-only workspace-creator role (host) |
