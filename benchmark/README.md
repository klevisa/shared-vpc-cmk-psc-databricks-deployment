...
against the **Dataproc** run of the *same input*, and measure cost and runtime per run.
**Owner:** Data Platform (+ the Airflow/Dataproc team for the observed Dataproc side).
...
The Dataproc jobs run in the background — we **observe** them, we don't submit them. **Airflow**
(Cloud Composer) is the orchestrator: it senses each Dataproc job finishing, copies that job's
**exact input** to `poc_bucket` (Storage Transfer Service), verifies the copy, then triggers the
Databricks Photon/Spark runs.

The Databricks side is a **Databricks Asset Bundle**: `databricks bundle deploy` pushes the jobs and
dashboard. **Airflow** triggers the runs.
...
The same PySpark file runs three ways; two are ours to run:
...
| 3 | Dataproc | Spark | **observed** — the background run; we don't submit it |
...
Per benchmarked Dataproc job:

1. **`wait_for_dataproc`** — downstream of the `DataprocSubmitJobOperator` (or a `DataprocJobSensor`).
   Also **captures runtime** from `status_history` (RUNNING→terminal) — Airflow owns the job, so no
   collector grant is needed. This is why the collector never calls Dataproc.
...
3. **`sts_copy`** — Storage Transfer Service, **manifest-driven**, into a run-scoped prefix
   `gs://poc_bucket/benchmark/<benchmark_run_id>/`. STS checksum-verifies every object.
4. **`verify_fingerprint`** (gate) — recompute the fingerprint on the destination; **fail the DAG if
   it differs**. This is the "same exact set of files" guarantee — Databricks won't fire on a bad copy.
...
The benchmarked Dataproc job runs on an **ephemeral cluster labeled `poc=photon-poc` +
...
The source dataset is live, so "copy the prefix" would race additions/rewrites. Instead the DAG pins
the **exact object set** the Dataproc job read (a closed partition → immutable in practice;
...
**Attribution keys.** `poc=photon-poc` scopes the view (least privilege — the collector sees only our
clusters); `engine` splits **dataproc vs databricks** (and photon vs spark); the per-run key is the
**Dataproc cluster uuid** or the Databricks **`job_run_id`**. Compute Engine rows carry `poc` for
*both* platforms, so `engine` (not `poc`) is what tells Dataproc VM cost apart from Databricks VM
cost. `run_id` maps back to the tracking table to attach the manifest/fingerprint and Dataproc runtime.
...
Airflow mints a `benchmark_run_id` that ties everything together, written to
`analytics.benchmark.benchmark_runs` by the Databricks job (it has UC creds and knows its own
`job_run_id`):

| col | source | used for |
|---|---|---|
| `benchmark_run_id` | Airflow run_id | joins everything |
| `dataproc_job_id`, `cluster_uuid` | Airflow (job placement) | Dataproc VM+license via uuid in the view |
...
| `sts_job_id` | Databricks job (from params) | copy cost line |
| `dbx_job_run_id` | Databricks job (self) | DBU + VM$ key |

### Photon coverage

In-job (**Approach A**): `sample_job` walks the executed physical plan, counts `Photon*` operator
nodes vs the total, and appends `photon_coverage_pct` + `photon_fallback_ops` to
`analytics.benchmark.photon_coverage` keyed by `run_id`; `collect_costs` joins it into `results`. The
**Spark baseline** comes out ~0%; **Dataproc** rows are NULL (no Photon engine).

### Copy overhead (a separate line)

Track the copy's cost — **STS transfer** and **`poc_bucket` storage** for the duplicated dataset — as
a **distinct** bucket (label the STS job / `poc_bucket` `poc=photon-poc`; it lands in the same view as
its own `cost_bucket`). Report it *separately* so the engine comparison stays clean.

```mermaid
flowchart TB
    A["Airflow DAG<br/>sense → manifest+fingerprint → STS copy → verify → trigger"]
...
Photon-coverage, and the copy-overhead line. It deploys with the bundle.

## Identities (run-as, separation of duties)

One GCP service account and three Databricks service principals. Airflow (its Composer SA + a
Databricks trigger credential) drives the orchestration; people only view results.
...
dashboard/
  benchmark.lvdash.json   the Lakeview dashboard (Photon vs Spark vs Dataproc + copy overhead)
prerequisites.md          Phase 5 setup: billing export/view, the collector SA, SPs, Airflow/STS
airflow/                  the DAG (sense → manifest → STS → verify → trigger)
```
...
# 2) deploy the DAG to Cloud Composer (airflow/). Per Dataproc job it:
#    senses completion + duration → captures manifest + fingerprint → STS copy to poc_bucket
...
- **Attribution is concurrency-proof and cross-platform:** DBU is keyed by `job_run_id`; every VM
  carries `poc` + `engine` + a per-run key, so one scoped view separates Dataproc VM/PD/license from
  Databricks VM/PD. Extract labels as **scalar subqueries** and group on a single key — the flattened
  export repeats cost once per label (Dataproc stamps three), so `CROSS JOIN UNNEST` + `SUM` would
  triple-count.
