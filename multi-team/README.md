...
|---|---|---|---|
| Phase 0 | [`foundation/`](foundation) | **Cloud Foundation** | service project, API enablement, Shared VPC host+attach, GCS service agent |
...
```
Phase 0 foundation ─► service project, APIs, Shared VPC       (Cloud Foundation)
     ┌──────────────┴──────────┐
Phase 1 host-network          Phase 2 service-cmek   parallel  (Network Eng / Security)
                      └─► Phase 3 databricks-account            (Data Platform)
                               └─► Phase 4 post-workspace       (Network / IAM)
```

Phase 0 comes first. Phases 1 and 2 are then independent. Phase 3 consumes both. Phase 4
consumes Phases 1 and 3. After Phase 3, re-check the Phase-1 PSC status outputs — they
flip **PENDING → ACCEPTED** once the endpoints are registered.
