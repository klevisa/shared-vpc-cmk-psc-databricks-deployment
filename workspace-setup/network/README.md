...
- **No internet egress** — NPIP cluster VMs reach Google APIs (GCS, KMS) via **Private Google Access** and the Databricks control plane via the **backend PSC endpoint**, so the private path needs none. If clusters must pull **public** packages (PyPI / npm / Maven), the customer can add a Cloud Router + NAT, or point installers at private mirrors reachable over the VPC — see Additional info.
...
**No internet egress by default.** Nothing on the private path needs it — Google APIs go over Private Google Access and the Databricks control plane over the backend PSC endpoint, so a workspace with no internet egress still functions. If teams must install **public** packages (PyPI / npm / Maven), the customer can add a **Cloud Router + NAT**, or point pip / npm / Maven at **private** mirrors reachable inside the VPC (or via PSC / VPC peering / interconnect) — typically via a cluster init script or env vars like `PIP_INDEX_URL`. If adding NAT, make sure the firewall and routes reach the index hosts, and that anything Databricks itself pulls is mirrored too.
...
Finally, the **static subnet grant** gives the service project's Compute Engine service agent `compute.networkUser` on the node subnet — the permission that lets a VM owned by the *service* project be placed on a subnet owned by the *host* project. The Databricks **workspace service account** needs subnet access too, but it doesn't exist until the workspace is created, so that grant (the narrow network role) happens in step 2.6.
